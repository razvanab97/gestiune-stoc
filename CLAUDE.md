...
- `research_projects` / `research_links` — competitor research projects and their scraped candidate links (`research_projects.visual_dna` holds the AI-computed "Product Visual DNA" used by the gallery generator, see below)
- `product_generated_images` — planned/generated frames for the "Galerie AI produs" feature, one row per gallery image, scoped by `project_id` → `research_projects(id)` (not by `produse.id` — see below)
...
`api/ai-image.js` is separate — it calls OpenAI's Images API directly (`/v1/images/edits`, model from `OPENAI_IMAGE_MODEL` env var, default `gpt-image-1`), not the Responses API proxy above. POST with `sourceImageUrl(s)`+`prompt` does an image-edit (real product photo as source of truth) and stores the base64 result in the `ai_images` table; POST with `imageData` is a plain upload (no AI); GET serves a stored image back as a stable `/api/ai-image?id=N` URL. Used by `recreateImageWithAI` (single-image cleanup) and the whole "Galerie AI produs" pipeline (multi-image reference, see above).
...
- **Galerie AI produs** (in the Research listing panel, `renderListingFactoryResult`, only when `listingProjectId` is set): generates an ~8-photo e-commerce gallery from real product photos. Pipeline: `computeVisualDna` (vision `callAI` call → "Product Visual DNA" JSON — what the AI must NOT change) → `createGalleryPlan` (`callAI` → array of frame plans, adaptive per product type) → `generateOneGalleryImage`/`generateAllGalleryImages` (concurrency-limited at `GALLERY_MAX_CONCURRENT=2`, calls `/api/ai-image` with multiple reference images) → `validateGalleryImage` (vision `callAI` comparing generated vs. original, 0-100 identity score). All state lives in `galleryFor(projectId)` (module-level `galleryState` map) and persists to `product_generated_images`; approved frames flow into `produse.images` via `applyListingToProduct`. Known gaps (see changelog #163): no canvas/SVG text-overlay templates yet (dimension arrows/benefit callouts), reordering is ↑/↓ buttons not drag-and-drop, bulk download is sequential not a ZIP.
